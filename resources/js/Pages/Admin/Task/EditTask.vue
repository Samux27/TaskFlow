<template>
            ...
            <form @submit.prevent="submitForm" enctype="multipart/form-data">
              ...
              <div class="mb-4">
                ...
                <input
                  ...
                  class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 p-2"
                />
                <div v-if="form.errors.dead_line" class="text-red-500 text-sm mt-1">{{ form.errors.dead_line }}</div>
              </div>
  ...
  // Carga datos al montar o si cambia la prop
  const resetForm = () => {
    if (!props.task) return
    form.title = props.task.title || ''
    form.description = props.task.description || ''
    form.employe_id = props.task.employe_id || ''
    form.boss_id = props.bossId || 1
    form.create_date = props.task.create_date ? props.task.create_date.slice(0, 10) : new Date().toISOString().slice(0, 10)
    form.dead_line = props.task.dead_line ? props.task.dead_line.slice(0, 10) : ''
    form.importancia = props.task.importancia || 'media'
    form.estado = props.task.estado || 'pendiente'
    form.assigned_users = props.task.assigned_users?.map(u => u.id) || []
  }
  ...
  // Opcional: debug para ver datos enviados
  ...
  <style scoped>
  /* Aquí puedes personalizar el estilo */
  </style>
