        ...
        <div class="bg-white p-6 rounded-lg shadow border border-gray-200">
          ...
          <form @submit.prevent="submitForm">
            ...
            <div class="mb-4">
              ...
              <input v-model="form.title" id="title" type="text" autocomplete="off"
            ...
            <!-- ──────────── PICKER DE USUARIOS ──────────── -->
            <UserPicker v-if="showModal" v-model:show="showModal"
                        v-model:selected="form.assigned_users" :users="employees" />
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <input v-model="form.create_date" id="create_date" type="date"
                       class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 p-2" />
              ...
              <div>
                ...
                <input v-model="form.dead_line" id="dead_line" type="date"
                       class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 p-2" />
                <small v-if="form.errors.dead_line" class="text-red-500">{{ form.errors.dead_line }}</small>
              </div>

              <!-- Fecha de finalización SOLO cuando estado = finalizada -->
              <div v-if="form.estado === 'finalizada'">
                <label for="complete_at" class="block text-gray-700">Fecha de finalización</label>
                <input v-model="form.complete_at" id="complete_at" type="date"
                       class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 p-2" />
                <small v-if="form.errors.complete_at" class="text-red-500">{{ form.errors.complete_at }}</small>
              </div>
            </div>
...
watch(() => form.estado, (val) => {
  if (val !== 'finalizada') form.complete_at = ''
})
...
const resetForm = () => {
  ...
  form.complete_at    = t.complete_at?.slice(0, 10) ?? ''
